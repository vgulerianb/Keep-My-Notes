  ...
  try {
    ...
    if (request["code"]) {
      try {
        ...
        bodyFormData.append("client_secret", keys.google.clientSecret);
        bodyFormData.append("redirect_uri", keys.google.redirect);
        bodyFormData.append("client_id", keys.google.clientId);
        ...
        if (token?.data?.id_token) {
          ...
          const appToken = jwt.sign({ user: email }, keys.jwtSecret);
          res.status(200).send({
            status: "success",
            message: "Success",
            token: appToken,
...
router.delete("/notes", token.verifyToken, async (req, res) => {
  try {
    ...
    if (request?.["id"] && request["id"] !== "") {
      ...
      res.status(400).send({ status: "error", message: "Id is requered." });
  } catch (err) {
    res.status(500).send({
      status: "error",
      message: "Server error",
